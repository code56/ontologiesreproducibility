...
def argbuilder():
    parser = argparse.ArgumentParser(
        epilog="Note: Starred (*) arguments are disabled by default and will produce VERBOSE results if enabled.")
    parser.add_argument("pdf", help="Source PDF")
    parser.add_argument("-d", help="Dump all stream objects to a specified location", dest="location",
                        metavar="LOCATION")
    parser.add_argument("-s", help="Set maximum size of file in megabytes. Default is 2.", dest="max_size", default=2)
    parser.add_argument("-p", help="Specify PDF user/open password (owner passwords are ignored)", dest="password",
                        metavar="PASSWORD")
    parser.add_argument("--no_summary", help="Showing the summary is the default. This disables it.",
                        action="store_true")
    parser.add_argument("--show_json", help="Outputs pdf in json to the screen. Disabled by default.",
                        action="store_true")
...
def main():
    ...
    if not args.location is None:
        if pydf2json.os.path.isdir(args.location):
            ...
            print
            '%s is not a valid dump directory location.' % args.Location
            exit()
    ...
    try:
        ...
        e
        print
        'See command help for max_size override.'
        exit()
    ...
    if args.show_json:
        print
        json.dumps(jsonpdf, ensure_ascii=False, indent=4)
    ...
    if not args.no_summary:
        ...
        if summary['Encryption']['enabled']:
            print
            '{:<29} {:<32}'.format('Encrypted:', 'True')
            print
            '{:<29} {:<32}'.format('User Pass:', args.password)
            print
            '{:<29} {:<32}'.format('File Key:', summary['Encryption']['file_key'])
            print
            '{:<29} {:<32}'.format('Key Length:', str(summary['Encryption']['key_length']) + ' bits')
            print
            '{:<29} {:<32}\n'.format('Algo:', summary['Encryption']['algorithm'])
        else:
            print
            '{:<20} {:>10}'.format('Encrypted:', 'False')
        aa_num = 0
        aa_sections = {}
        ...
        print
        '{:<20} {:>10}'.format('Additional Actions:', str(aa_num))
        print
        '{:<20} {:>10}'.format('AcroForms:', str(summary['AcroForms']))
        print
        '{:<20} {:>10}'.format('Embedded Files:', str(len(summary['EmbeddedFiles'])))
        print
        '{:<20} {:>10}'.format('JS:', summary['JS'])
        print
        '{:<20} {:>10}'.format('Launch:', str(len(summary['Launch'])))
        print
        '{:<20} {:>10}'.format('Object Streams:', str(summary['Object Streams']))
        print
        '{:<20} {:>10}'.format('OpenActions:', str(summary['OpenActions']))
        print
        '{:<10} {:>20}'.format('Pages:', str(summary['Pages']))
        ...
        for i in jsonpdf['Document Hashes']:
            print
            '\t{:<10} {:<0}'.format(i, jsonpdf['Document Hashes'][i])
        ...
        if int(tmp_index, 16) > 0:
            ...
            for i in range(0, len(summary['Malware Index'])):
                ...
                if tmp_num > 9:
                    if i == 7:
                        print
                        '\t' + str(tmp_num) + ' instances of unnecessary white space.'
                if tmp_num > 0:
                    if i == -1:  # Disabled with -1. Reenable by setting to 5
                        print
                        '\t' + str(tmp_num) + ' misaligned objects or cross-reference tables.'
                    if i == 6:
                        print
                        '\t' + str(tmp_num) + ' unnecessary encodings of named objects. Anti-analysis technique.'
