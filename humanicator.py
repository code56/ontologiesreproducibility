...
genbank_pattern = re.compile(
    r'\b([HNTRWUBIGS]{1}[0-9]{5}|(?:A[AIWFY]{1}|B[EFGIMQU]{1}|C[ABDFKNOVX]{1}|D[NRTVWYQ]{1}|E[BCEGHLSVWXYFU]{1}|F[CDEFGKLJ]{1}|G[DEHORTWQU]{1}|H[OSMQ]{1}|J[GKZFNQX]{1}|K[CFJMPRTUXY]{1})[0-9]{6}|[A-Z]{4}[0-9]{8,9}|[A-Z]{5}[0-9]{7}\.[0-9]{1,3})')
...
def detectogenome(bigrams, phases, title):
    ...
    for bigram in bigrams:
        if str.lower(bigram[0][0]) == 'complete' and (
                str.lower(bigram[0][1]).endswith('genome') or str.lower(bigram[0][1]).endswith('genomic')):
    ...
    if not abstract:
        ...
        if args.genome:
            ...
            if detected:
                ...
                seq_list = ['GenBank', 'ID', 'No.', 'no.', 'ENA', 'SRA', 'NCBI', 'genome', 'accession', 'annotation',
                            'DDBJ', 'EMBL', 'deposited']
                ...
                print("Accessions:")
                print("Refseq: {0}".format(
                    ['https://www.ncbi.nlm.nih.gov/refseq/?term={}'.format(m) for m in refseq_matches]))
                print(
                    "INSDC: {0}".format(['https://www.ncbi.nlm.nih.gov/sra/?term={}'.format(m) for m in insdc_matches]))
                print("GenBank: {0}".format(
                    ['https://www.ncbi.nlm.nih.gov/nuccore/{}'.format(m) for m in genbank_matches]))
