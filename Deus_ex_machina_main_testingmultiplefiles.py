...
def main(articlename):
        ...
        if len(xml_metadata) == 0:
            ...
            for query, onto_id in po_dict.items():
                if query in xml_metadata[1]:
                    print('found matches between PO onto dict and xml metadata:', query, onto_id) #these should be tabulated as well.
    ...
    #header = ['name', 'ngram matches in the article', 'project accession number','score for ontology matching at xml file', 'ontology matches between XML file and ontology database', 'score for finding Project Accession number']
    data = [[file1.name, ngram_matches, project_accession, score_xml, xml_and_onto_matching, score_for_finding_project_accession_number]]
...
'''
#I want to skip the headers everytime I run the code, so the headers do not get repeated. 
    with open('scores.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
...
'''
with open('scoring.csv', 'rb') as infile, open('scoring.csv', 'w') as outfile:
    reader = csv.reader(infile)
    next(reader, None)  # skip the headers
    writer = csv.writer(outfile)
    for row in reader:

    with open("scores.csv", "r") as infile, open("scores.csv", "w") as outfile:
        reader = csv.reader(infile)
        next(reader, None)  # skip the headers
        writer = csv.writer(outfile)
        for row in reader:
